...
> [!NOTE]
> Options should be sorted lexicographically (`download,jpg`; not `jpg,download`) to produce a deterministic, canonical URL for a given set of image options. This improves cache hit rates across browsers and CDNs. URLs generated by `blurhash-url` are sorted automatically.
...
Blurhash URL-compatible components render images in three layers. The first layer renders immediately, reserving space in the DOM for the incoming image and preventing cumulative layout shift. Then client JS then paints a Blurhash layer on top, which shows until the full-resolution image finishes loading and is placed on the top layer.

This improves both perceived image load time and measured metrics like those reported by PageSpeed Insights.
